node{
    ...
    stage('Build Docker Container'){
        if (branch == 'develop'){
           ...
           app = docker.build("${env.DEV_REGISTRY_URL}/${env.STAG_NAMESPACE}/customer-dashboard:0.1", "-f ./dockerfiles/sit.Dockerfile .")
        }
    }
    ...
    stage('Deploy image'){
     ...
     sh "kubectl config set-cluster ${env.ICP_CLUSTER} --server=https://${env.ICP_CLUSTER_IP} --insecure-skip-tls-verify=true"
                  ...
                  sh "kubectl config set-context ${env.ICP_CLUSTER}-context --user=luyennv --namespace=${env.STAG_NAMESPACE}"
                  sh "kubectl config use-context ${env.ICP_CLUSTER}-context"
                  sh "kubectl set image deployments/customer-dashboard customer-dashboard=${env.DEV_REGISTRY_URL}/${env.STAG_NAMESPACE}/customer-dashboard:${env.TGT_DEV_VERSION}-${BUILD_NUMBER} --namespace=${env.STAG_NAMESPACE}"
        }
      }
