      ...
      <div class="success-container">
        <div class="success-content">
          ...
          <v-layout align-center>
            <v-flex>
              <div class="big-title">{{$t('audience.success_submit_name.title')}}</div>
              <div class="semi-title">
                {{$t('audience.success_submit_name.info')}}
                <span class="color-main">{{audience.name}}</span>
                {{$t('audience.success_submit_name.info2')}}
              </div>
              <div class="button-container">
                <v-btn type="submit" class="cbt-btn accent-main" @click="closeDialog">{{$t('dialog.actions.close')}}</v-btn>
              </div>
            </v-flex>
          </v-layout>
        </div>
